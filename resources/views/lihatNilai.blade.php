<x-layout>
    ...
    <div class="justify-between items-center mb-6">
        ...
        <h2 class="text-xl font-extrabold tracking-normal leading-normal text-balance text-[#c60d0d]">
        ...
        {{-- Riwayat --}}
        <div class="hidden overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center bg-gray-900/30 backdrop-blur-md"
            id="modal-{{ $data['id'] }}">
            <div class="relative w-auto my-6 mx-auto max-w-3xl min-w-[500px]">
                {{-- Content --}}
                <div
                    class="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                    {{-- Header --}}
                    <div class="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                        <h3 class="text-2xl font-semibold text-[#0B132B]">
                            Riwayat Evaluasi: {{ $data['project_title'] }}
                        </h3>
                        <button onclick="toggleModal('modal-{{ $data['id'] }}')" type="button"
                            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-2 ml-auto inline-flex items-center transition-all duration-200"
                            title="Tutup Riwayat">
                        ...
                        @if (count($data['riwayat']) > 0)
                            <table class="w-full text-sm text-left text-gray-500">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3">PG</th>
                                        <th class="px-6 py-3">Essay</th>
                                    </tr>
                                </thead>
                                <tbody>
                            ...
                            <div class="border-t border-gray-100 pt-4 mt-4">
                                <p class="text-xs font-bold text-gray-400 uppercase mb-2 flex items-center gap-2">
                                    ...
                                    Feedback Essay (Terbaru)
                                </p>
                                <div
                                    class="bg-gray-50 rounded-lg p-4 text-sm text-gray-700 leading-relaxed border border-gray-100">
                                    @if (!empty($data['best']['feedback']) && $data['best']['feedback'] !== 'Belum ada feedback.')
                                        "{{ $data['best']['feedback'] }}"
                                    @else
                                        <span class="text-gray-400 italic">Belum ada feedback essay.</span>
                                    @endif
                                </div>
                            </div>
                        @else
                            <p class="text-center text-gray-500 py-4">Belum ada riwayat pengerjaan.</p>
                        @endif
                    </div>
                </div>
            </div>
