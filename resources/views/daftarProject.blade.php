                ...
                @foreach ($projects as $project)
                    @if ($project->project_bisa_akses)
                        <a href={{ url('/project/' . $project->slug) }}
                            ...
                            <span> Pertemuan {{ $loop->iteration }}: {{ $project->title }}</span>
                        </a>
                    @else
                        <div onclick="alert('{{ $project->pesan }}')"
                            class= "rounded-md bg-gray-200 px-10 py-3 text-lg font-extrabold text-gray-500 shadow-xs flex items-center cursor-pointer opacity-60">
