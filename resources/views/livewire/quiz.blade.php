<div>
    @if ($status == 'instruksi')
        <div class="max-w-6xl w-full bg-white rounded-lg border border-gray-300 shadow-sm place-self-center mt-10 mb-10">
            <div class="p-4 border-b border-gray-200">
                <h1 class="text-2xl font-bold text-[#0B132B]">Evaluasi {{ $project->title }}</h1>
            </div>
            <div class="p-6 text-justify">
                <div>
                    <h2 class="text-2xl font-bold text-green-600 mb-2">Kerja bagus!</h2>
                ...
                <div class="mb-3">
                    <h3 class="text-xl font-bold text-[#0B132B] mb-3">Selanjutnya: Evaluasi Fase {{ $project->title }}
                    </h3>
                ...
                <div class="mb-3">
                    <h4 class="text-lg font-bold text-[#0B132B] mb-3">Petunjuk Pengerjaan:</h4>
    ...
    @elseif ($status == 'mulai' && $question)
        <h1 class="mt-5 ml-3 mb-5 text-4xl font-bold text-[#0B132B]">Evaluasi {{ $project->title }}</h1>
        <div class="flex flex-col md:flex-row gap-4 mb-10 mt-8">
            <div class="w-full md:w-1/4 p-6 bg-white rounded-lg border border-gray-300 shadow-sm ml-3 mr-3">
                <h1 class="text-2xl font-bold text-[#0B132B]">Navigasi Soal</h1>
                <div class="grid grid-cols-5 gap-3 mt-5">
                    @foreach ($idQuestions as $idQuestion)
                        ...
                        <button wire:click="lompatSoal({{ $loop->index }})"
                            class="p-2 h-12 border-2 border-[#E49273] rounded-md text-center font-bold transition-colors {{ $buttonClass }}">
                            {{ $loop->iteration }}
                        </button>
                    @endforeach
                </div>
            </div>
            <div class="w-full md:w-3/4 p-6 bg-white rounded-lg border border-gray-300 shadow-sm mr-3 ml-3">
                <div class="p-4 bg-[#E49273] rounded-2xl">
                    <h1 class="text-2xl font-bold text-[#0B132B] text-center">Soal
                        {{ $QuestionIndex + 1 }}/{{ $totalPertanyaan }}
                    </h1>
                </div>
                <div class="p-6 bg-[#E8F1F2] rounded-md mt-4 mb-4">
                    <p class="text-[#0B132B] text-lg font-semibold mb-4">{{ $question->question }}</p>
                </div>
                <div>
    ...
    @if ($konfirmasiSubmit)
        <div class="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
                <div class="p-6">
                    <h3 class="text-xl font-bold text-gray-900 mb-2">Konfirmasi Akhiri Evaluasi</h3>
